...
2. [Tempest v3 API](#tempest-v3-api)
...
Tempest v3 is a cryptographic CSPRNG with 2^256 key-recovery security (MILP-verified). It uses a pure GF(2) round function with $\{\text{AND},\text{XOR},\text{ROT}\}$ operations only (no integer ADD or carry chains): Phase A (Weyl per-round key), Phase B (4-source XOR+AND nonlinear diffusion + snapshot ANDs), Phase C (Pre-mix + 4-level cross-word AND cascade, deg×16/round), Phase D (cross-word mixing). Security bounds: $\beta_1\geq 16$, $\beta_{r\geq2}=256$, $a_{\min}\geq 7$ (MILP), $\mathrm{DP}^{(25)}\leq 2^{-175}$, $c^{(25)\,2}\leq 2^{-150}$. Passes NIST SP 800-22 (15/15), TestU01 all 5 levels, and PractRand (1 TiB, zero anomalies). Single-core throughput: 17.7 Gbit/s (76.5 Gbit/s AVX-512 on Zen 4).
...
Performs one round of the Tempest v3 round function, then extracts a single 64-bit output through the dual-andmix4 output pipeline: fold4 linear projection ($t = u\oplus\texttt{rotl}(v,32)\oplus w\oplus\texttt{rotl}(z,16)$), dual Pre-mix diffusion, andmix4 cascade ($\deg\times16$), half-xor compression ($t\oplus(t\gg 32)$). Algebraic degree after the output function is $\beta_1\geq 16$ (proven); after 2 rounds, $\beta_{r\geq2}=256$ (algebraic completeness, MILP-validated).

**Parameters**:
- `s` -- pointer to an initialized `tempest_state`. Must not be NULL.

**Return value**: A cryptographically random 64-bit unsigned integer. Under Hypotheses H1 and H2, the output is computationally indistinguishable from uniform random with 2^256 security.
...
Each index `j` in the range `[i, n-1]` is chosen using `tempest_range(s, i, n-1)`, ensuring uniform permutation selection with no modulo bias. The resulting permutation is computationally indistinguishable from a uniformly random permutation under the 2^256 security target.
...
```c
typedef struct {
    uint64_t u, v, w, z, r, weyl;
} tempest_state;
```

Six 64-bit state words (u,v,w,z + round counter + Weyl sequence value). Total 48 bytes. The state is opaque: always access through the API functions.
