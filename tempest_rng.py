...
class _TempestLibrary:
    ...
    def _load_single_ops(self):
        ...
        if tv3_path and os.path.exists(tv3_path):
...
class TempestRNG:
    ...
    def __init__(self, key=None, nonce=None, seed=None):
        ...
        if seed is not None and key is None:
            ...
            n = [
                (seed ^ 0x9E3779B97F4A7C15) & M,
                ((~seed & M) + 0x6A09E667F3BCC908) & M
            ]
            self._key = b''.join(x.to_bytes(8, 'little') for x in k)
            self._nonce = n[0].to_bytes(8, 'little') + n[1].to_bytes(8, 'little')
            seed_actual = list(k)
        else:
            if key is None:
                key = os.urandom(32)
    ...
    def reseed(self, entropy=None):
        """重新注入熵 — 全新初始化（与主密钥编排相同的 22 轮）
        SP 800-90A 类接口：从新熵派生 key+nonce 并调用 tempest_init。"""
        if entropy is None:
            entropy = os.urandom(48)  # 48 字节 = 32 key + 16 nonce
        elif len(entropy) < 48:
            entropy = entropy + os.urandom(48 - len(entropy))

        k_arr = (ctypes.c_uint64 * 4)(
            int.from_bytes(entropy[0:8], 'little'),
            int.from_bytes(entropy[8:16], 'little'),
            int.from_bytes(entropy[16:24], 'little'),
            int.from_bytes(entropy[24:32], 'little'))
        n_arr = (ctypes.c_uint64 * 2)(
            int.from_bytes(entropy[32:40], 'little'),
            int.from_bytes(entropy[40:48], 'little'))
        self._lib.tempest_init(ctypes.byref(self._state), k_arr, n_arr)
        self._key = bytes(entropy[:32])
        self._propagate_state()
    ...
    def clear_key(self):
        """安全清零存储的密钥（Python bytes 不可变性使这仅清除引用）"""
        self._key = b'\x00' * 32
    ...
    def __repr__(self):
        return (f"TempestRNG(security=2^128)")
