...
static void seed_from_os(uint64_t key[4], uint64_t nonce[2]) {
    ...
    if (fp) {
        if (fread(key, 1, 32, fp) < 32) { goto fallback; }
        if (fread(nonce, 1, 16, fp) < 16) { goto fallback; }
        fclose(fp);
        return; /* /dev/urandom succeeded — use it directly */
    }
#endif
fallback:
    /* Weak fallback: only ~30 bits entropy for demo purposes */
