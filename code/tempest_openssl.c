...
/* DRBG-style Update: XOR data → 8 rounds mix.
   NOTE: This is a CSPRNG with SP 800-90A-like interface, NOT a certified
   SP 800-90A DRBG. Lacks CRNGT, adaptive proportion tests, and KAT
   auto-verification required for FIPS 140 certification. */
    ...
    size_t entropy_len, const OSSL_PARAM params[])
{
    ...
    if (!ctx) return 0;
    if (!entropy || entropy_len < 48) return 0;  /* SP 800-90A: need >= 48 for 128-bit security */

    /* 从熵输入派生 key (256-bit) + nonce (128-bit) — 最小 48 字节熵 */
    memset(key, 0, sizeof(key));
    memset(nonce, 0, sizeof(nonce));
    memcpy(key, entropy, 32);
    ...
    const unsigned char *adin, size_t adinlen)
{
    ProvCtx *ctx = (ProvCtx *)vctx;
    (void)strength;
    if (prediction_resistance) return 0;  /* not supported — reject request */
    ...
    if (remain > 0) {
        ...
        size_t copy = remain < sizeof(r) ? remain : sizeof(r);
        memcpy(ptr, &r, copy);
    }
    ...
    const unsigned char *adin, size_t adinlen)
{
    ...
    if (!ctx || !entropy || entropy_len < 48) return 0;

    /* 构造 256-bit 更新数据 (entropy_len >= 48 已保证) */
...
/* uninstantiate: 清理 DRBG 状态 */
static int tempest_uninstantiate(void *vctx) {
    ProvCtx *ctx = (ProvCtx *)vctx;
    if (!ctx) return 0;
    OPENSSL_cleanse(ctx, sizeof(*ctx));
    ctx->initialized = 0;
...
static const OSSL_DISPATCH tempest_rand_dispatch[] = {
    ...
    { OSSL_FUNC_RAND_UNINSTANTIATE,      (void (*)(void))tempest_uninstantiate },
...
static const OSSL_ALGORITHM tempest_algorithms[] = {
    { "TEMPEST-DRBG",                   /* 算法名 — EVP_RAND_fetch 用 */
      ...
      "Tempest v3 CSPRNG — DRBG-like interface, 2^128 security" /* 描述 — 类 DRBG 接口，未经 FIPS 140 认证 */
    },
