...
static void tempest_round(TempestState *s) {
    ...
    uint64_t wval = s->weyl; wval += WEYL_GOLDEN;
    ...
    u = andmix4(u); z = andmix4(z); /* dual andmix4 */
    ...
    const uint64_t key[4], const uint64_t nonce[2])
{
    ...
    for (int i = 0; i < 16; i++) {
        ...
        if (i < 8) {
            ...
            s->u ^= n0; s->v ^= ROTL(n1, 19) ^ (uint64_t)i;
            s->z ^= ROTL(n0, 43);
        }
    }
...
static int TempestBitGen_init(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (key_str && strlen(key_str) == 64) {
        ...
        uint64_t nonce[2] = {s ^ 0x9E3779B97F4A7C15ULL, ~s + 0x6A09E667F3BCC908ULL};
        tempest_init(&obj->state, key, nonce);
    }
    return 0;
}
...
static PyObject *TempestBitGen_get_capsule(PyObject *self, void *closure) {
    ...
    BitGenCapsule *bg = (BitGenCapsule *)PyMem_Malloc(sizeof(BitGenCapsule));
    if (!bg) {
