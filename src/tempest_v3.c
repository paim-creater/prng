...
static void enhanced_round(tx4_state *s) {
    ...
    /* Phase A: Weyl with nonlinear filter */
    uint64_t wv = s->weyl + WEYL_GOLDEN;
    ...
    /* Phase B: XOR replaces ADD + AND covers cross-word pairings (pure GF(2)) */
    u = u0 ^ rotl(v0,5) ^ rotl(w0,17) ^ (rotl(v0,5) & rotl(z0,25)) ^ K_U;   /* XOR(u,w) + AND(v,z) */
    v = v0 ^ rotl(w0,11) ^ rotl(z0,23) ^ (rotl(w0,11) & rotl(u0,29)) ^ K_V;   /* XOR(v,z) + AND(w,u) */
    w = w0 ^ rotl(z0,13) ^ rotl(u0,31) ^ (rotl(u0,9) & rotl(v0,15)) ^ K_W;
    z = z0 ^ rotl(u0,17) ^ rotl(v0,7) ^ (rotl(v0,27) & rotl(w0,21)) ^ K_Z;

    /* Phase B(lin): snapshot ANDs covering (u,z) and (w,z) for linear resistance */
    u ^= (rotl(z0, 23) & rotl(w0, 53));   /* AND (z,w) */
    z ^= (rotl(u0, 5) & rotl(z0, 25));    /* AND (u,z) */

    /* Phase C: Pre-mix + 4-level cross-word AND cascade */
...
void tempest_init(tempest_state *s, const uint64_t key[4], const uint64_t nonce[2]) {
    ...
    for(int i=0;i<16;i++){
        enhanced_round(s); kw+=WEYL_GOLDEN;
        if(i<8){
            if(i&1){
