...
static void print_reference(void) {
    ...
    for (int i = 0; i < 5; i++)
        printf("%016llX ", (unsigned long long)tempest_u64(&s));
...
int main() {
    ...
    tempest_state s, s2;
    tempest_init(&s, key, nonce);
    tempest_init(&s2, key, nonce);
    int det_ok = 1;
    for (int i = 0; i < 100000; i++)
        if (tempest_u64(&s) != tempest_u64(&s2)) { det_ok = 0; break; }
    printf("Determinism:  %s\n", det_ok ? "PASS" : "FAIL");
    ok = det_ok && ok;

    /* Bit balance (10^8 bits)  */
    tx5cmul_seed(&s, 42);  /* seed for deterministic test sequences */
    long ones = 0, N = 100000000L;
    for (long i = 0; i < N / 64; i++)
        ones += popcnt(tempest_u64(&s));
    ...
    tempest_init(&s, key, nonce);
    tempest_init(&s2, key2, nonce);
    long diff = 0;
    for (int i = 0; i < 10000; i++)
        diff += popcnt(tempest_u64(&s) ^ tempest_u64(&s2));
    ...
    tempest_init(&s, zero_key, zero_nonce);
    uint64_t first = tempest_u64(&s);
    uint64_t second = tempest_u64(&s);
    ...
    /* KAT verification (key={1,2,3,4}, nonce={5,6})  */
    tempest_state kat_s;
    int kat_ok = (tempest_kat_verify(&kat_s) == 0);
    printf("KAT verify:   %s\n", kat_ok ? "PASS" : "FAIL");
    ok = kat_ok && ok;
