...
//! ## Performance
//!
//! - ~17.7 Gbit/s throughput (single core, Zen 4, pure GF(2))
//! - ~3.0× faster than ChaCha20
//! - Passes NIST STS 15/15, TestU01 (all 5 suites), PractRand 1 TiB
//! - Conservative security strength: 2^128
//!
//! ## Algorithm
//!
//! Tempest v3 uses a pure GF(2) round function with only XOR, ROTL, and
//! AND operations (no integer ADD/CMUL). The 4-stage AND-mix cascade doubles
//! algebraic degree 4 times per round — proven by induction.
//! After 2 rounds: deg ≥ 256, XL complexity ≥ 2^345 (heuristic estimate).
//! AND is GF(2) multiplication, making the degree growth fully analyzable.
...
impl TempestState {
    ...
    fn key_schedule(&mut self, key: [u64; 4], nonce: [u64; 2]) {
        ...
        for i in 0..16i32 {
            ...
            if i < 8 {
                ...
                let n0 = nonce[(i & 1) as usize];
                let n1 = nonce[1 - (i & 1) as usize];
                self.u ^= n0;
                self.v ^= rotl(n1, 19) ^ (i as u64);
                self.z ^= rotl(n0, 43);
            }
        }
    ...
    pub fn round(&mut self) {
        ...
        let u0 = u; let v0 = v; let w0 = w; let z0 = z;
    ...
    /// AND-mix: 4-stage cascade, each stage doubles GF(2) degree.
    #[inline(always)]
    fn andmix4(t: u64) -> u64 {
        let mut r = t;
        r ^= rotl(r, 31) & rotl(r, 53);
        r ^= rotl(r, 17) & rotl(r, 43);
        r ^= rotl(r, 7) & rotl(r, 23);
        r ^= rotl(r, 5) & rotl(r, 19);
        r
    }

    /// Output function: pure GF(2), strictly provable.
    #[inline(always)]
    fn output(&self) -> u64 {
    ...
    /// Second output uses permuted state: φ(v,w,z,u) — inline, no temporary.
    #[inline]
    pub fn next_u64x2(&mut self) -> [u64; 2] {
        self.round();
        let out0 = self.output();
        let mut t = self.v ^ rotl(self.w, 32) ^ self.z ^ rotl(self.u, 16);
        t ^= rotl(t, 27) ^ rotl(t, 17);
        t = Self::andmix4(t);
        t ^= t >> 32;
        [out0, t]
    ...
    pub fn fill_bytes(&mut self, buf: &mut [u8]) {
        ...
        if i < buf.len() {
            ...
            let mut remaining = buf.len() - i;
            let mut offset = 0;
            while remaining >= 8 {
                let r2 = self.next_u64();
                buf[i + offset..i + offset + 8].copy_from_slice(&r2.to_le_bytes());
                offset += 8;
                remaining -= 8;
            }
            if remaining > 0 {
                buf[i + offset..].copy_from_slice(&bytes[..remaining]);
            }
        }
    }
...
///
/// **⚠️ WARNING:** Although `TempestRng` implements [`CryptoRng`], seeding via
/// [`SeedableRng::from_seed`] provides at most **64 bits of entropy**. The
/// output from `from_seed` is **deterministic and reproducible** — it is NOT
/// suitable for cryptographic key generation, session tokens, or any security
/// context where unpredictability is required.
///
/// For cryptographic use, always use [`TempestRng::new(key, nonce)`] with a
/// 256-bit key from a secure entropy source (e.g., `getrandom`).
///
/// ## Example (correct)
///
/// ```rust,ignore
/// // GOOD: 256-bit key + 128-bit nonce from secure source
/// let mut key = [0u8; 32];
/// let mut nonce = [0u8; 16];
/// getrandom::fill(&mut key).unwrap();
/// getrandom::fill(&mut nonce).unwrap();
/// let rng = TempestRng::new(&key, &nonce);  // true cryptographic security
/// ```
///
/// ## Example (wrong — only 64-bit security)
///
/// ```rust,ignore
/// // BAD: only 64 bits of entropy
/// let mut rng = TempestRng::from_seed(42u64.to_le_bytes());
/// let key: [u8; 32] = rng.gen_bytes();  // attacker can brute-force seed!
/// ```
...
impl Default for TempestRng {
    ...
    fn default() -> Self {
        ...
        if getrandom::fill(&mut key).is_err()
            || getrandom::fill(&mut nonce).is_err()
        {
            // getrandom 失败是严重错误——不静默降级
            panic!("TempestRNG: getrandom failed to obtain entropy");
        }
        Self::new(&key, &nonce)
    }
    ...
    #[cfg(not(feature = "std"))]
    fn default() -> Self {
        panic!("TempestRNG: no_std requires explicit seeding via from_seed() or new()");
    }
}
